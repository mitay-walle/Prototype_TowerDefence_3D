...
namespace TD
{
	...
	public class TurretGenerationProfile : GenerationProfile
	{
		...
		private void AddChamferedBox(Part part,
		                             Vector3 position,
		                             int width,
		                             int height,
		                             int depth,
		...
		private void GenerateBasePlate(Part basePlate)
		{
			...
			for (int y = cachedFeetHeight + 1; y < cachedFeetHeight + baseBorderThickness; y++)
			{
				for (int side = 0; side < 4; side++)
				{
					for (int i = -halfSize + 1; i < halfSize; i++)
					{
						...
						basePlate.voxels.Add(new VoxelData
						{
		...
		private void GenerateRotationRing(Part basePlate)
		{
		...
		private void GenerateTurretBody(Part turret)
		{
		...
		private void GenerateBarrelHousing(Part turret)
		{
			...
			AddChamferedBox(turret, new Vector3(0, baseHeight + housingHeight / 2, cachedBodyDepth / 4), housingWidth, housingHeight, housingDepth, 2,
				3);
		}

		private void GenerateBarrel(Part turret)
		{
			...
			if (cachedBarrelCount == 1)
			{
				...
				AddChamferedBox(turret, new Vector3(xOffset, barrelY, startZ + barrelLength / 2), barrelThickness, barrelThickness, barrelLength, 0,
					4);
				...
				if (hasBarrelAttachment)
				{
					...
					AddChamferedBox(turret, new Vector3(xOffset, barrelY - barrelThickness / 2 - attachmentSize / 2, attachmentPos),
			...
			else if (cachedBarrelCount == 2)
			{
				...
				for (int side = -1; side <= 1; side += 2)
				{
					...
					if (hasSuppressor)
					{
						...
						int suppressorThickness = barrelThickness + 2;
						AddChamferedBox(turret, new Vector3(side * spacing, barrelY, startZ + barrelLength + suppressorLength / 2),
							suppressorThickness, suppressorThickness, suppressorLength, 0, 1);
					}
					else
					{
					...
					int supportLength = RandomRangeInt(6, 12);
					AddChamferedBox(turret, new Vector3(side * spacing, barrelY - 2, startZ - supportLength / 2), barrelThickness + 2, 3,
						supportLength, 1, 1);
				}
			}
			else
			{
				...
				for (int i = -1; i <= 1; i++)
				{
					AddChamferedBox(turret, new Vector3(i * spacing, barrelY, startZ + barrelLength / 2), barrelThickness, barrelThickness,
						barrelLength, 0, 4);
					...
					AddChamferedBox(turret, new Vector3(i * spacing, barrelY - 2, startZ - supportLength / 2), barrelThickness + 2, 2, supportLength,
						1, 1);
				}
			}
		}

		private void GenerateSensors(Part turret)
		{
			...
			for (int i = 0; i < sensorCount; i++)
			{
				...
				AddChamferedBox(turret, new Vector3(xPos, yPos + sensorSize / 2, zPos), sensorSize, sensorSize, sensorSize, 1, i % 2 == 0 ? 6 : 7,
					i % 2, RandomRange(1.5f, 3f));
			}
		}

		private void GeneratePowerCore(Part turret)
		{
		...
		private void GenerateArmorPanels(Part turret)
		{
			int halfWidth = cachedBodyWidth / 2;
			int halfDepth = cachedBodyDepth / 2;
		...
		private void GenerateCoolingVents(Part turret)
		{
		...
		private void GenerateFuelTanks(Part turret)
		{
