	...
	protected override void OnInspector()
	{
		...
		if (Any(t => t.Volume == true))
		{
			BeginIndent();
				...
				{
					BeginIndent();
						BeginError(Any(t => t.FadeIn + t.FadeOut > t.Duration));
							DrawDefault("FadeIn");
						EndError();
						BeginError(Any(t => t.FadeIn + t.FadeOut > t.Duration));
							DrawDefault("FadeOut");
...
public class VA_AudioSource : MonoBehaviour
{
	...
	[Tooltip("Set duration of playing")]
	[SerializeField]
	...
	protected virtual void LateUpdate()
	{
		...
		if (VA_Helper.GetListenerPosition(ref listenerPosition) == true)
		{
			...
			if (Volume == true)
			{
				...
				{
					var step = Mathf.InverseLerp(0f, 1f, (Time.time - _startTime) / FadeIn);
					finalVolume *= step;
				}
				
				if (FadeOut > 0f && Time.time > _startTime + (Duration - FadeOut) && Time.time < _startTime + Duration)
				{
					var step = Mathf.InverseLerp(1f, 0f, (Time.time - (_startTime + (Duration - FadeOut))) / FadeOut);
					finalVolume *= step;
				}
