...
- Механические проверки команд и локальных tool calls вынесены в `.codex/hooks.json`: hook блокирует msbuild, опасные git reset/clean/worktree и restore/checkout открытой сцены, ручное изменение Unity serialized assets через shell/apply_patch, generic Unity reflection calls, scene reload и блокирующие Unity dialogs; открытие и сохранение сцен разрешены; owner-chain, документация и runtime-proof остаются обязательными правилами этого файла.
...
- Для изменённых C# используй `mcp-unity-validate-script`, затем дождись автоматической компиляции Unity. Если автоматическая компиляция не сработала, зависла или нужна повторная диагностика, вручную запускай `TD/Automation/Force Recompile All`.
