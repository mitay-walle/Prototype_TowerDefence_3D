...
- Удаляй [Tooltip] где не очевидно, но выноси строку в константу
...
- удаляй [BoxGroup]
- рефактори вложенные классы и структуры
- назначай ссылки и настраивай конфиги, создавай файлы настроек, префабы, сцены
- удаляй документацию
...
- используй {get; private set;}
- используй TMP_Text TextMeshProUGUI вместо Text
- используй UniTask вместо Coroutine
- UGUI объекты вкладывай иерархически внутрь скриптов, которым они пренадлежат (фасад)
...
- сначала вызови вручну рекомпайл. дождить окончания компиляции, потом проверяй наличие ошибок
- все соединения вокселей дорог по XZ должны иметь соседние клетки, чтобы быть проходимыми
- не создавай .asmdef без разрешения